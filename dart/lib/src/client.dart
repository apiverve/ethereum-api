...
class EthereumValidationException implements Exception {
  final List<String> errors;

  EthereumValidationException(this.errors);

  @override
  String toString() => 'EthereumValidationException: ${errors.join("; ")}';
}
...
/// Ethereum Price API client.
///
/// For more information, visit: https://apiverve.com/marketplace/ethereum?utm_source=dart&utm_medium=readme
///
/// Parameters:
...
class EthereumClient {
  ...
  EthereumClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/ethereum',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [EthereumValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw EthereumValidationException(errors);
    }
  }

  /// Execute a request to the Ethereum Price API.
  ///
  /// Parameters are validated before sending the request.
  Future<EthereumResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw EthereumException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return EthereumResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw EthereumException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw EthereumException('Resource not found');
      } else {
        throw EthereumException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is EthereumException) rethrow;
      throw EthereumException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Ethereum Price API client.
class EthereumException implements Exception {
  final String message;

  EthereumException(this.message);
